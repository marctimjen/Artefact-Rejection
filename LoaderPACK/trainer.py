...
from LoaderPACK.Accuarcy_finder import Accuarcy_find, Accuarcy_find_elec
from LoaderPACK.Accuarcy_upload import Accuarcy_upload, Accuarcy_upload_elec
...
def net_multi_train(device,
    ...
    for iEpoch in range(nEpoch):
        ...
        t_mat = torch.zeros(3, 2, 2) # save confusion matrix
        total_pos, total_neg = torch.tensor(0), torch.tensor(0)
        ...
        run[f"{net_name}/matrix/val_confusion_matrix_pr_file"].log(v_mat)
        Accuarcy_upload_elec(run, v_mat, total_pos, total_neg, f"{net_name}", "val")
